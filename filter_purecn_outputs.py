...
for line in f1:
    if line[0] == "#":
        if "FILTER" in line and filter_add == False:
            f2.write("##FILTER=<ID=PASS,Description=\"Considered as somatic variants by PureCN and pass GDC filtering\">\n")
            f2.write("##FILTER=<ID=purecn_call,Description=\"Considered as somatic variants by PureCN but do not pass GDC filtering\">\n")
            filter_add = True
        f2.write(line)
        continue
    ...
    if filter == ".":
        if info["PureCN.PS"] >= prob_thres:
            f2.write("\t".join(val[0:6] + ["PASS"] + val[7:]) + "\n")
        else:
            f2.write("\t".join(val[0:6] + ["purecn_call"] + val[7:]) + "\n")
    else:
        f2.write("\t".join(val[0:6] + [filter + ";purecn_call"] + val[7:]) + "\n")
f2.close()
f1.close()
