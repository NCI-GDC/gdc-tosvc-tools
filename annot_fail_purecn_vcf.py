...
parser.add_argument('-l', '--purecn_log', type=str, required=True, help='purecn log file')
parser.add_argument('-i', '--input_vcf', type=str, required=True, help='input vcf file')
parser.add_argument('-o', '--output_vcf', type=str, required=True, help='annotated output vcf file')
args = vars(parser.parse_args())
log_fn = args['purecn_log']
...
for line in f1:
    if line[0] == "#":
        if filter_add == False:
            ...
            if error_capture:
                f2.write("##gdc_filtering_status=PureCN 1.11.11 cannot find valid purity/ploidy solution. These calls have been run through PureCN-GDCfiltration tumor-only variant calling pipeline without PureCN filtering.\n")
            else:
                f2.write("##gdc_filtering_status=PureCN 1.11.11 fails to run on this sample. These calls have been run through PureCN-GDCfiltration tumor-only variant calling pipeline without PureCN filtering.\n")
            filter_add = True
        f2.write(line)
        continue
